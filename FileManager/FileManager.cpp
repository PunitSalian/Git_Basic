...
template <typename T>
Result<std::monostate, ErrorCode> removeFile(const T &param)
{
...
template <typename T>
Result<size_t, ErrorCode> getFilesize(const T &param)
{
...
Result<std::monostate, ErrorCode> storage::FileManager<T>::readFile(const T &param, std::span<uint8_t> buffer)
{
    ...
    file.read(reinterpret_cast<uint8_t *>(buffer.data()), f.value());
